...
then
  ...
  git clone "${_pyenv_git:-https://github.com/pyenv/pyenv.git}" "$PYENV_ROOT"
fi
eval "$(pyenv init -)"
...
then
  ...
  then
    ...
    git clone "${_pyenv_venv_git:-https://github.com/pyenv/pyenv-virtualenv.git}" "$PYENV_ROOT/plugins/pyenv-virtualenv"
  fi
